...
local function CheckItemsAndStopFarming()
    ...
    if AreBothItemsObtained() then
        if not ItemsObtainedNotified then
            print("All required items obtained! Starting Auto Pull Lever...")
            ...
            -- Start Auto Pull Lever
            if getgenv().PullLeverSettings and getgenv().PullLeverSettings['Pull Lever'] then
                print("Pull Lever setting is enabled! Starting auto pull lever...")
                spawn(function()
                    print("Creating spawn for StartAutoPullLever...")
                    StartAutoPullLever()
                end)
            else
...
if AreBothItemsObtained() and getgenv().StopFarmingWhenItemsObtained then
    print("Both items already obtained! Starting Auto Pull Lever immediately...")
    ItemsObtainedNotified = true
    ...
    if getgenv().PullLeverSettings and getgenv().PullLeverSettings['Pull Lever'] then
        spawn(function()
            wait(3) -- Wait a short time to ensure GUI is set up
            StartAutoPullLever()
        end)
    else
        print("Pull Lever setting is disabled. Not starting auto pull lever.")
